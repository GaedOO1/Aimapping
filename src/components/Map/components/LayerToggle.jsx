  ...
  return (
    <LayerToggleContainer $isCollapsed={isLayerMenuCollapsed}>
      ...
      <ToggleButton 
        ...
        onClick={async () => {
          if (!showMUDLayer) {
            await addMUDLayer(map.current);
          } else {
            removeMUDLayer(map.current);
          }
      ...
      <ToggleButton 
        $active={showHoustonCensusBlocks}
        onClick={async () => {
          if (!showHoustonCensusBlocks) {
            await addHoustonCensusBlocksLayer(map.current);
          } else {
            removeHoustonCensusBlocksLayer(map.current);
          }
          setShowHoustonCensusBlocks(!showHoustonCensusBlocks);
        }}
        style={{ height: '32px', padding: '0 12px', fontSize: '14px', marginBottom: '8px' }}
      >
        {showHoustonCensusBlocks ? 'Hide Census Blocks' : 'Show Census Blocks'}
      </ToggleButton>
